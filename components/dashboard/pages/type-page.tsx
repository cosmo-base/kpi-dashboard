    ...
    return (
      <div className="w-full min-w-[280px] py-1">
        <div className="flex justify-between text-[11px] font-bold mb-1.5 gap-3">
          <span className="w-1/2 truncate text-left text-[#38BDF8]">
            {item.options[0].label}
          </span>
          <span className="w-1/2 truncate text-right text-[#8B5CF6]">
    ...
    Promise.all([
      ...
      .then(([csv1, csv2, csv3]) => {
        ...
        let thisMonthTotal = 0,
          thisMonthSimpleTotal = 0,
          thisMonthSimpleCbhp = 0,
          thisMonthSimplePart = 0,
          thisMonthDetailed = 0;
        let thisWeekTotal = 0,
          thisWeekSimpleTotal = 0,
          thisWeekSimpleCbhp = 0,
          thisWeekSimplePart = 0,
          thisWeekDetailed = 0;
        let todayTotal = 0,
          todaySimpleTotal = 0,
          todaySimpleCbhp = 0,
          ...
          questionsList.forEach((q) => {
            ...
            if (validCount > 0) {
              ...
              if (isScale) {
                ...
                let sortOrder = q.key.startsWith("Q")
                  ? 100 + parseInt(q.key.replace("Q", ""), 10)
                  : ["R", "D", "I", "O", "P"].indexOf(q.key) + 1;
        ...
        if (sortedMapRecords.length > 0) {
          ...
          while (currentDateObj.getTime() <= endDateObj.getTime()) {
            ...
            const dw =
              currentDateObj.getDay() === 0 ? 7 : currentDateObj.getDay();
        ...
        const detailedTypeDistribution = Array.from(detailedTypeMap.entries())
          .sort(
            (a, b) => detailedOrder.indexOf(a[0]) - detailedOrder.indexOf(b[0]),
          )
          .map(([k, v]) => ({
            name: k,
            value: v,
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-2">
          {Object.entries(DETAILED_TYPE_LABELS).map(([key, label]) => {
            // 集計済みのデータから人数を取得
            const count =
              charts.detailedTypeDistribution?.find((d: any) => d.name === key)
                ?.value || 0;

...
const CSV_URLS = {
  trend:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=1015522883&single=true&output=csv",
  source:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=1037755345&single=true&output=csv",
  pages:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=1528131378&single=true&output=csv",
  region:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=2107194814&single=true&output=csv",
  conversions:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=1531874959&single=true&output=csv",
  demographics:
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vQEI0LS0PR12MT59Iiiy3EFC9ja4bPjMMfn6F1IIYp4IhrQh_9s7gJKu5uRmVGeleXWWOn05V-VZ7RN/pub?gid=1984427854&single=true&output=csv",
};
...
export function ParticipantsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([trendTxt, sourceTxt, pagesTxt, regionTxt, convTxt, demoTxt]) => {
        ...
        // --- 3. コンバージョン(イベント)の処理 (ページごとに全イベントを紐づける) ---
        ...
        convRows.forEach((row) => {
          const evName = String(row.eventName || "不明");
          const path = row.pagePath || "不明";
          const count = parseInt(row.eventCount, 10) || 0;
          totalEvents += count;
          eventsByPage.set(path, (eventsByPage.get(path) || 0) + count);
          eventNameMap.set(evName, (eventNameMap.get(evName) || 0) + count);
        });

        const eventDistribution = Array.from(eventNameMap.entries())
          .sort((a, b) => b[1] - a[1])
          .map(([name, value], i) => ({
            name,
            value,
        ...
        const pageData = Array.from(pathMap.values()).map((item) => {
          const eventCount = eventsByPage.get(item.path) || 0;
          const cvr = item.totalPv > 0 ? (eventCount / item.totalPv) * 100 : 0;
          const monthRate =
            item.prevMonth === 0
              ? item.thisMonth > 0
                ? 100
        ...
        // --- 6. 性別・年齢層の処理 ---
        const demoRows = Papa.parse(demoTxt, {
          header: true,
          skipEmptyLines: true,
        }).data as any[];
        ...
        const genderData = Array.from(genderMap.entries())
          .map(([name, value], i) => ({
            ...
            value,
            color:
              name === "男性" ? "#38BDF8" : name === "女性" ? "#EC4899" : "#6B7280",
          }))
          .sort((a, b) => b.value - a.value);
  ...
  const sortedPageData = [...tables.pageDataRaw].sort((a, b) => {
    if (sortMode === "events_desc") return b.eventNum - a.eventNum;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <ScrollableTable
          columns={[
            ...
            { key: "eventStr", label: "イベント数", align: "right" },
          ...
          data={sortedPageData.map((item, i) => ({
            ...
            eventStr:
              item.eventNum > 0 ? (
                <span className="text-info font-bold">{item.eventNum}回</span>
              ) : (
                "-"
              ),
            cvrComp:
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <SectionCard title="イベント種別の割合" description="発生したイベントの内訳">
          <ChartContainer height="h-[300px]">
            {charts.eventDistribution && charts.eventDistribution.length > 0 ? (
              <DonutChart data={charts.eventDistribution} centerLabel="イベント" />
            ) : (
              <div className="flex h-full items-center justify-center text-muted-foreground">
                データがありません
              </div>

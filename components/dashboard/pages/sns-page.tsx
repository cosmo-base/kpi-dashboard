...
export function SNSPage() {
    ...
    fetch(csvUrl)
      ...
      .then(csvText => {
        ...
        const calculateMetrics = (key: 'x' | 'instagram' | 'note') => {
          ...
          const latest = dailyRecords[len - 1][key];
          const getPrevValue = (daysAgo: number) => len > daysAgo ? dailyRecords[len - 1 - daysAgo][key] : dailyRecords[0][key];
          const prev1 = getPrevValue(1);
          const prev7 = getPrevValue(7);
          const prev30 = getPrevValue(30);

          return {
            followers: latest,
            todayIncrease: latest - prev1,
            weeklyIncrease: latest - prev7,
            weeklyRate: prev7 === 0 ? 100 : Math.round((latest / prev7) * 100),
            monthlyIncrease: latest - prev30,
            monthlyRate: prev30 === 0 ? 100 : Math.round((latest / prev30) * 100)
          };
        };
        ...
        processedRecords.forEach(record => {
          ...
          if (!monthlyMap.has(monthLabel)) {
            monthlyMap.set(monthLabel, { x: 0, instagram: 0, note: 0, total: 0 });
          }
        ...
        if (processedRecords.length > 0) {
           const firstMonth = parseInt(processedRecords[0].date.split('/')[0], 10);
           if (firstMonth > now.getMonth() + 1 + 1) currentYear--;
        }
        ...
        processedRecords.forEach(record => {
           ...
           if (!weeklyMap.has(weekKey)) {
              weeklyMap.set(weekKey, { 
                x: 0, instagram: 0, note: 0, total: 0,
    ...
    return (
      <div className="flex h-[400px] items-center justify-center text-muted-foreground">
        <div className="animate-pulse flex items-center gap-2">
          ...
          <span>スプレッドシートからSNSデータを読み込み中...</span>
        </div>
      </div>
    );
  }
  ...
  const getTrendLines = () => {
    switch (filterMode) {
      ...
      default: return [
        { dataKey: '総数', color: '#F59E0B', name: '総数' },
        { dataKey: 'X', color: '#1DA1F2', name: 'X' },
        { dataKey: 'Instagram', color: '#d640e4', name: 'Instagram' },
        { dataKey: 'note', color: '#41C9B4', name: 'note' },
      ];
    }
  };
  ...
  const getMonthlyBars = () => {
    switch (monthlyFilterMode) {
      ...
      default: return [
        ...
        { dataKey: 'Instagram', name: 'Instagram', color: '#d640e4' },
        { dataKey: 'note', name: 'note', color: '#41C9B4' },
      ];
    }
  };
  ...
  const chartFilterButtons: { mode: ChartFilterMode; label: string; color: string }[] = [
    { mode: 'total', label: '総数', color: 'bg-[#F59E0B]' },
    { mode: 'x', label: 'X', color: 'bg-[#1DA1F2]' },
    { mode: 'instagram', label: 'Instagram', color: 'bg-[#d640e4]' },
    { mode: 'note', label: 'note', color: 'bg-[#41C9B4]' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-foreground">SNS分析</h2>
          <p className="text-sm text-muted-foreground">X、Instagram、noteのフォロワー数と増加推移を確認できます</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 rounded-xl bg-[#1DA1F2]/10 border border-[#1DA1F2]/20">
              <XIcon className="h-5 w-5 text-[#1DA1F2]" />
            </div>
            <span className="text-lg font-semibold text-foreground">X (Twitter)</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            <KpiCard title="フォロワー数" value={platforms.x.followers.toLocaleString()} icon={Users} accentColor="primary" />
            <KpiCard title="1カ月増加数" value={formatDiff(platforms.x.monthlyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="1カ月比" value={platforms.x.monthlyRate} unit="%" trendValue={`前月比`} trendType="up" icon={Calendar} accentColor="primary" />
            <KpiCard title="週間増加数" value={formatDiff(platforms.x.weeklyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="週間比" value={platforms.x.weeklyRate} unit="%" trendValue={`前週比`} trendType="up" icon={Zap} accentColor="accent" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 rounded-xl bg-[#d640e4]/10 border border-[#d640e4]/20">
              <InstagramIcon className="h-5 w-5 text-[#d640e4]" />
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            ...
            <KpiCard title="1カ月増加数" value={formatDiff(platforms.instagram.monthlyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="1カ月比" value={platforms.instagram.monthlyRate} unit="%" trendValue={`前月比`} trendType="up" icon={Calendar} accentColor="primary" />
            <KpiCard title="週間増加数" value={formatDiff(platforms.instagram.weeklyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="週間比" value={platforms.instagram.weeklyRate} unit="%" trendValue={`前週比`} trendType="up" icon={Zap} accentColor="accent" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 rounded-xl bg-[#41C9B4]/10 border border-[#41C9B4]/20">
              <NoteIcon className="h-5 w-5 text-[#41C9B4]" />
            </div>
            <span className="text-lg font-semibold text-foreground">note</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            <KpiCard title="フォロワー数" value={platforms.note.followers.toLocaleString()} icon={Users} accentColor="success" />
            <KpiCard title="1カ月増加数" value={formatDiff(platforms.note.monthlyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="1カ月比" value={platforms.note.monthlyRate} unit="%" trendValue={`前月比`} trendType="up" icon={Calendar} accentColor="primary" />
            <KpiCard title="週間増加数" value={formatDiff(platforms.note.weeklyIncrease)} icon={TrendingUp} accentColor="success" />
            <KpiCard title="週間比" value={platforms.note.weeklyRate} unit="%" trendValue={`前週比`} trendType="up" icon={Zap} accentColor="accent" />
      ...
      <SectionCard title="SNSフォロワー推移" description="フォロワー数推移（媒体別切替可能）">
        <div className="flex flex-wrap gap-2 mb-4">
          {[
            ...
            { mode: 'instagram' as FilterMode, label: 'Instagramのみ', color: 'bg-[#d640e4]' },
            { mode: 'note' as FilterMode, label: 'noteのみ', color: 'bg-[#41C9B4]' },
          ].map((btn) => (
            <Button
              key={btn.mode}
              variant="outline"

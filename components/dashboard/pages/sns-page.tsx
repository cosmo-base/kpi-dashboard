...
export function SNSPage() {
    ...
    fetch(csvUrl)
      ...
      .then((csvText) => {
        ...
        const dailyRecords = rawData
          ...
          .filter(
            (record) => record.x > 0 || record.instagram > 0 || record.note > 0,
          );
        ...
        if (
          ...
          parseInt(dailyRecords[0].date.split("/")[0], 10) > currentM + 1
        )
          currentYearForDate--;
        ...
        const recordsWithDateNum = processedRecords.map((record) => {
          ...
          if (prevMonthForDate !== -1 && prevMonthForDate === 12 && m === 1)
            currentYearForDate++;
          prevMonthForDate = m;
          return {
            ...record,
        ...
        const calculateMetrics = (key: "x" | "instagram" | "note") => {
          ...
          return {
            ...
            weeklyRate:
              endOfPrevWeekVal === 0
                ? 100
                : Math.round((latest / endOfPrevWeekVal) * 100),
            monthlyIncrease: thisMonthInc,
            monthlyRate:
              endOfPrevMonthVal === 0
                ? 100
                : Math.round((latest / endOfPrevMonthVal) * 100),
          };
        };
        ...
        const monthlyMap = new Map<
          string,
          {
            ...
            cumulative: number;
            cumulativeX: number;
            cumulativeInstagram: number;
            cumulativeNote: number;
          }
        >();
        processedRecords.forEach((record) => {
          ...
          cur.cumulative = record.total;
          cur.cumulativeX = record.x;
          cur.cumulativeInstagram = record.instagram;
          cur.cumulativeNote = record.note;
        });
        ...
        const monthlyTableRaw = Array.from(monthlyMap.entries()).map(
          ([month, val]) => ({
            ...
            cumulativeX: val.cumulativeX,
            cumulativeInstagram: val.cumulativeInstagram,
            cumulativeNote: val.cumulativeNote,
          }),
        );
        const monthlyTable = [...monthlyTableRaw].reverse();
        const monthlyByPlatform = [...monthlyTableRaw]
          .slice(-4)
          .map((item) => ({
            name: item.month,
            X: item.x,
        ...
        if (
          ...
          parseInt(processedRecords[0].date.split("/")[0], 10) >
            nowJst.getMonth() + 2
        )
          currentYear--;
          ...
          growthHistorySeries: [
            {
              ...
              label: "総数",
              color: "#F59E0B",
              history: monthlyTableRaw.map((item) => ({
                month: item.month,
            ...
            },
            {
              ...
              history: monthlyTableRaw.map((item) => ({
                month: item.month,
                cumulative: item.cumulativeInstagram,
              })),
            },
            {
              key: "note",
              ...
              history: monthlyTableRaw.map((item) => ({
                month: item.month,
                cumulative: item.cumulativeNote,
              })),
            },
          ],
        });
  ...
  const { platforms, charts, tables, growthHistorySeries } = data;
  const getTrendLines = () => {
    switch (filterMode) {
      case "x":
        return [{ dataKey: "X", color: "#1DA1F2", name: "X" }];
      case "instagram":
        return [{ dataKey: "Instagram", color: "#d640e4", name: "Instagram" }];
      case "note":
        return [{ dataKey: "note", color: "#41C9B4", name: "note" }];
      default:
        return [
  ...
  const getMonthlyBars = () => {
    switch (monthlyFilterMode) {
      case "x":
        return [{ dataKey: "X", name: "X", color: "#1DA1F2" }];
      case "instagram":
        return [{ dataKey: "Instagram", name: "Instagram", color: "#d640e4" }];
      case "note":
        return [{ dataKey: "note", name: "note", color: "#41C9B4" }];
      default:
        return [
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 border-b border-border/50 pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            X、Instagram、noteのフォロワー数と増加推移を確認できます
          </p>
        </div>
        <Button
          asChild
      ...
      <div className="mt-8 border-t border-border/50 pt-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            過去の月間平均増加数（線形ペース）をもとに算出した、SNSフォロワー数の12ヶ月先までの予測値です。
          </p>
        </div>

        <GrowthProjectionSection
          title="SNSフォロワー数の成長予測"
          description="X・Instagram・noteそれぞれの直近の月次増加ペースから、今月末〜1年後までの想定フォロワー数を算出します。「全体を重ねて表示」で媒体別に比較、各ボタンで単体表示に切り替えられます。"
          unit="人"

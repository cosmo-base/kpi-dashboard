      ...
      .then((csvText) => {
        ...
        const dailyRecords = rawData
          ...
          .map((row) => {
            const snsTotal =
              (Number(row[xIdx]) || 0) +
              (Number(row[instagramIdx]) || 0) +
              (Number(row[noteIdx]) || 0);
        ...
        recordsWithDateNum.forEach((r) => {
          if (r.recordNum >= startOfThisMonthNum)
            thisMonthIncrease += r.increase;
        ...
        const weeklyIncreaseRate =
          endOfPrevWeekTotal === 0
            ? 100
            : Math.round((latest.total / endOfPrevWeekTotal) * 100);

        const monthlyMap = new Map<
          string,
          { increase: number; total: number }
        >();
        ...
        const monthlyTable = monthlyTableRaw.map((item, idx) => {
          ...
          if (idx > 0) {
            ...
            rate =
              prevIncrease <= 0
                ? 100
                : Math.round((item.increase / prevIncrease) * 100);
          }
          return {
            month: item.month,
        ...
        if (
          ...
          parseInt(dailyRecords[0].date.split("/")[0], 10) >
            nowJst.getMonth() + 2
        )
          currentYear--;
        ...
        <SectionCard title="月別増加人数" description="直近4か月の推移">
          <ChartContainer height="h-[300px]">
            <StackedBarChart
              ...
              bars={[
                { dataKey: "増加数", name: "増加人数", color: "#38BDF8" },
                { dataKey: "減少数", name: "減少人数", color: "#EF4444" },
              ]}
            />
          </ChartContainer>
        </SectionCard>
        <SectionCard title="週別増加人数" description="直近4週の推移 (月〜日)">
          <ChartContainer height="h-[300px]">
            <StackedBarChart
              ...
              bars={[
                { dataKey: "増加数", name: "増加人数", color: "#8B5CF6" },
                { dataKey: "減少数", name: "減少人数", color: "#EF4444" },
              ]}
            />
          </ChartContainer>

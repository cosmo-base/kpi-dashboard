...
const getMonthColor = (monthLabel: string) => {
...
export function CBEDPage() {
    ...
    fetch(csvUrl)
      ...
      .then((csvText) => {
        ...
        const headerRowIndex = rawData.findIndex(
          (row) => row.includes("title") || row.includes("location"),
        );
        if (headerRowIndex === -1) return;

        const headers = rawData[headerRowIndex];
        const titleIdx =
          headers.indexOf("title") !== -1 ? headers.indexOf("title") : 1;
        const dateIdx =
          headers.indexOf("date") !== -1 ? headers.indexOf("date") : 2;
        const latIdx =
          headers.indexOf("lat") !== -1 ? headers.indexOf("lat") : 6;
        const lngIdx =
        ...
        const validRows = rawData
          ...
          .filter(
            (row) => row[titleIdx] && String(row[titleIdx]).trim() !== "",
          );
        ...
        validRows.forEach((row) => {
          ...
          if (updateStr) {
            ...
            if (
              ...
              updateDate.getTime() >= startOfToday.getTime()
            ) {
              todayUpdateCount++;
            }
          }
          ...
          const isOnline =
            String(row[latIdx] || "").trim() === "" &&
            String(row[lngIdx] || "").trim() === "";
          let region = "その他";
          ...
          if (isUpcoming) {
            upcomingCount++;
            upcomingMonthCounts.set(
              monthLabel,
              (upcomingMonthCounts.get(monthLabel) || 0) + 1,
            );
            if (isOnline) {
              upcomingRegionCounts.set(
                "オンライン",
                (upcomingRegionCounts.get("オンライン") || 0) + 1,
              );
            } else {
              upcomingRegionCounts.set(
                region,
                (upcomingRegionCounts.get(region) || 0) + 1,
              );
            }
          }
        });
        ...
        const upcomingRegionDistribution = Array.from(
          upcomingRegionCounts.entries(),
        )
          .map(([name, value]) => ({
            name,
            value,
        ...
        const upcomingMonthDistribution = Array.from(
          upcomingMonthCounts.entries(),
        )
          .filter(([name]) => name !== "未定")
          .map(([name, value]) => ({ name, value, color: getMonthColor(name) }))
          .sort((a, b) => (parseInt(a.name) || 0) - (parseInt(b.name) || 0));
        ...
        const onlineTable = [
          {
            ...
            upcoming: validRows.filter(
              (r) =>
                String(r[latIdx]).trim() === "" &&
                String(r[lngIdx]).trim() === "" &&
                new Date(String(r[dateIdx]).replace(/\//g, "-")) >= now,
            ).length,
  ...
  const sortedPrefectureTable = useMemo(() => {
    ...
    const undefinedItems = table.filter(
      (t) => !PREFECTURES_FULL.includes(t.prefecture) && t.listings > 0,
    );
    ...
    if (prefSortMode === "north") {
      validItems.sort(
        (a, b) =>
          PREFECTURES_FULL.indexOf(a.prefecture) -
          PREFECTURES_FULL.indexOf(b.prefecture),
      );
    } else {
      validItems.sort((a, b) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <KpiCard
          title="今月のイベント増減"
          value={
            summary.monthlyIncrease >= 0
              ? `+${summary.monthlyIncrease}`
              : summary.monthlyIncrease
          }
          unit="件"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <ChartContainer height="h-[300px]">
            {charts?.upcomingRegionDistribution?.length > 0 ? (
              <DonutChart
        ...
        >
          <ChartContainer height="h-[300px]">
            {charts?.upcomingMonthDistribution?.length > 0 ? (
              <DonutChart
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <SectionCard title="【全期間】 地方別＋オンライン割合">
          <ChartContainer height="h-[300px]">
        ...
        <SectionCard title="【全期間】 開催月ごとの件数割合">
          <ChartContainer height="h-[300px]">
